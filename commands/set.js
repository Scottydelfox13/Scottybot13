...
const newValue = args.slice(2);
  ...
  if (action === "edit") {
    ...
    if (!client.settings.has(message.guild.id)) client.settings.set(message.guild.id, {});
      ...
      if(defaults[key].type === "channel") {
        ...
        client.settings.set(message.guild.id, { value: `${chaan.name}` }, key);
        ...
        client.settings.set(message.guild.id, { value: `${rolE.name}` }, key);
        message.reply(`${key} successfully edited to ${rolE.name}`);
    } else {
    client.settings.set(message.guild.id, { value: `${joinedValue}` }, key); 
  ...
  if (action === "get") {
    ...
    client.settings.set(message.guild.id, { value: `${currentsetting}`, enabled: false }, key); 
    ...
    client.settings.set(message.guild.id, { value: `${currentsetting}`, enabled: true }, key); 
    ...
    Object.entries(settings).forEach(([key, value]) => {
      ...
      if(value.enabled) { enabled = `, ENABLED`; } else { enabled = ", DISABLED"; }
      }
