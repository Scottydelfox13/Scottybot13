exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
    ...
    switch (message.flags[0]) {
      ...
      case "list":
      default:
        const taglist = client.tags.filter(t=>t.guild===message.guild.id).map(t=>`${message.settings.prefix}${t.name}`).join("\n");
        if (taglist.length < 1) answer = ["There are no tags on this server.", null];
        else message.channel.send(`= List of Available Tags For ${message.guild.name} =\n\n${taglist}`, {code: "asciidoc"});
        break;
    }
