...
module.exports = (client, member) => {
  ...
  const welcomeMessage = settings.welcomeMessage.value.replace("{{user}}", member.user.username).replace("{{userMention}}", `<@${member.user.id}>`);

  // Send the welcome message to the welcome channel
  // There's a place for more configs here.
  
try {
member.guild.channels.find(c => c.name === settings.welcomeChannel.value).send(welcomeMessage);
} catch(err) {
console.log(err);
